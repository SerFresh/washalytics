...
class CustomerSerializer(serializers.ModelSerializer):
    class Meta:
        model = Customer
        fields = '__all__'  # ดึงทุกฟิลด์มาใช้
        extra_kwargs = {
            'password': {'write_only': True}  # ไม่ให้ส่ง password ออกไปใน API Response
        }
...
class MachineSerializer(serializers.ModelSerializer):
    customer = CustomerSerializer(read_only=True)  # แสดงข้อมูลลูกค้า
    customer_id = serializers.PrimaryKeyRelatedField(
        queryset=Customer.objects.all(), source='customer', write_only=True
    )  # รับ customer_id ตอนสร้าง

    class Meta:
        model = Machine
        fields = ['machine_id', 'model', 'machine_type', 'customer', 'customer_id', 'machine_status', 'machine_capacity', 'price_per_use']
