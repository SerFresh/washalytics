...
class Machine(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE)  # เชื่อมกับลูกค้า
    machine_status = models.CharField(max_length=50)
    machine_capacity = models.CharField(max_length=50, blank=True, null=True)
    price_per_use = models.IntegerField()

    def __str__(self):
        return f"{self.model} - {self.machine_type} ({self.machine_capacity})"
